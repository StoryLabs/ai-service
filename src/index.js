 ...
 * @param {string} args.model - id literal del modelo (ej. 'deepseek-v4-flash', 'muse-spark-1.2-contributor')
 * @param {Object} [args.config] - { temperature?, maxTokens?, timeoutMs? }
 * @param {number} [args.config.temperature] - 0..2, default por provider (DeepSeek 0.2)
 * @param {number} [args.config.maxTokens] - techo de completion_tokens, default por modelo
 * @param {number} [args.config.timeoutMs] - default por provider (DeepSeek 180_000)
 ...
 * @param {Object} [args.logger] - opcional, logger inyectable { warn, error, info }
...
export async function callAI({ messages, prompts, model, config = {}, userId = null, provider, logger: callerLogger, signal: callerSignal }) {
  ...
  if (messages && prompts) {
    ...
    log.warn('MOT-AI-015', 'callAI recibió messages y prompts — se usa messages')
  }
